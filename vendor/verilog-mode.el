...
;       verilog-highlight-p1800-keywords n
      ...
      (progn
        (let ( (pt (point)) (pass 0)) 
          ...
          (if (looking-at (concat "\\<constraint\\|coverpoint\\|cross\\|with\\>\\|" verilog-in-constraint-re))
            ...
            ;; sds--for coverpoints it will be the signal covered which can be a vector
            (if (looking-at "\\(\\[[0-9:.a-zA-Z_]*\\]\\)")
                (verilog-backward-token))
            ;; if first word token not keyword, it maybe the instance name
            ;;   check next word token
            (if (looking-at "\\<\\w+\\>\\|\\s-*(\\s-*\\w+|[a-zA-Z_]+\\(\\[[0-9:.a-zA-Z_]*\\]\\)*")
